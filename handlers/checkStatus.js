...
module.exports = function checkStatus({ client }) {
	...
	if (client.config.channel.startsWith('Put')) {
		console.log(chalk.cyan('[Raffi Solutions] ') + chalk.red('Error! Invalid Channel ID'))
		process.exit()
	} else if (client.config.panel.url.startsWith('Put')) {
		console.log(chalk.cyan('[Raffi Solutions] ') + chalk.red('Error! Invalid Panel URL'))
		process.exit()
	} else if (client.config.panel.key.startsWith('Put')) {
		console.log(chalk.cyan('[Raffi Solutions] ') + chalk.red('Error! Invalid Apikey'))
		process.exit()
	} else if (!client.config.panel.url.startsWith('http')) {
		console.log(chalk.cyan('[Raffi Solutions] ') + chalk.red('Error! Invalid Panel URL'))
		console.log(chalk.cyan('[Raffi Solutions] ') + chalk.red('1. Make sure the panel url is starts with "https://" or "http://"'))
		process.exit()
	}
	...
	console.log(chalk.cyan('[Raffi Solutions] ') + chalk.green('Getting nodes stats'))
		...
		axios(client.config.panel.url + '/api/application/users', {
			...
			if (error.response) {
				if (error.response.status === 403) {
					console.log(chalk.cyan('[Raffi Solutions] ') + chalk.red('Error! Invalid apikey'))
					console.log(chalk.cyan('[Raffi Solutions] ') + chalk.red('1. Make sure the apikey is from admin page not account page'))
					console.log(chalk.cyan('[Raffi Solutions] ') + chalk.red('2. Make sure the apikey has read permission on all options'))
					console.log(chalk.cyan('[Raffi Solutions] ') + chalk.red('3. Make sure the apikey is exist'))
				} else if (error.response.status === 404) {
					console.log(chalk.cyan('[Raffi Solutions] ') + chalk.red('Error! Invalid Panel URL'))
					console.log(chalk.cyan('[Raffi Solutions] ') + chalk.red('1. Make sure the panel url is like "https://panel.example.com"'))
				} else console.log(chalk.cyan('[Raffi Solutions] ') + chalk.red('Error! ' + error))
			} else console.log(chalk.cyan('[Raffi Solutions] ') + chalk.red('Error! ' + error))
			resolve()
		})
	})
				...
				axios(client.config.panel.url + '/api/application/nodes/' + node.attributes.id + '/configuration', {
					...
					const stats = new Promise((statsResolve, statsReject) => {
						axios(node.attributes.scheme + '://' + node.attributes.fqdn + ':' + node.attributes.daemon_listen + '/api/servers', {
							...
							if (client.config.log_error) console.log(chalk.cyan('[Raffi Solutions] ') + chalk.yellow('[Node: ' + node.attributes.name + '] ') + chalk.red(error))
						...
						setTimeout(() => {
							if (body.status === undefined) {
								if (client.config.log_error) console.log(chalk.cyan('[Raffi Solutions] ') + chalk.yellow('[Node: ' + node.attributes.name + '] ') + chalk.red('Timeout!'))
								embeds.push(Embed({ node: body }))
								body.status = false
								return statsResolve()
							}
