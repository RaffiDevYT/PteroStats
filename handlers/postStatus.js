...
module.exports = async function postStatus({ client, panel, nodes }) {
	...
	if (!channel) return console.log(chalk.cyan('[Raffi Solutions] ') + chalk.red('Error! Invalid Channel ID'))
	...
	stats.then(async () => {
		...
		console.log(chalk.cyan('[Raffi Solutions] ') + chalk.green('Stats posted!'))
	})
}